...
function resMember() {
	var con = confirm("정지된 회원을 복구하시겠습니까?");
	if(con) {
		location.href="memberRestore.do?id=${id}";
	}
}
</script>
            ...
            <div id="content">
                ...
                <div class="card shadow mb-4" style="width:600px; margin: 0 auto">
					<div class="card-header py-3">
						<h5 class="m-0 font-weight-bold text-primary">회원 정보</h5>
						<c:if test="${member.del eq 'n' }">
						<button class="btn btn-primary" onclick="delMember()" style="margin-top:-30px; float: right;">정지</button>
						</c:if>
						<c:if test="${member.del eq 'y' }">
						<button class="btn btn-primary" onclick="resMember()" style="margin-top:-30px; float: right;">복구</button>
						</c:if>
					</div>
					<div class="card-body">						
							<span>회원 이름: ${member.name }</span>
							<br>
							<span>회원 닉네임: ${profile.nick }</span>						
					</div>					
					<div class="card-body">	
					<c:if test="${profile.photo1 ne null }">
							<img width="100%" height="auto" id="photo" src="resources/profiles/${profile.photo1}" alt="이미지">
							<img width="100%" height="auto" id="photo" src="resources/profiles/${profile.photo2}" alt="이미지">
							<img width="100%" height="auto" id="photo" src="resources/profiles/${profile.photo3}" alt="이미지">
					</c:if>
					...
					<div class="card-body">
						<h4 class="card-title">선호 조건</h4>
						<ul class="list-group list-group-flush">
						<li class="list-group-item">선호하는 성별 : ${preference.gender }</li>
						<li class="list-group-item">선호하는 연령대 : ${preference.minAge }
							~ ${preference.maxAge }</li>
						<c:if test="${preference.tag != 'None' }">
							<li class="list-group-item">선호하는 관심사 : ${preference.tag }</li>
						</c:if>
						<c:if test="${preference.tag == 'None' }">
							<li class="list-group-item">선호하는 관심사 : 상관없음</li>
						</c:if>
						</ul>
					</div>
