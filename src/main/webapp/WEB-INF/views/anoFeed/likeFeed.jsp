								...
								<div class="card mb-3">
									...
									<div class="card-footer">
										<c:if test="${empty anoFeed.hashTag }">
											<span class="badge rounded-pill bg-secondary">설정된
												해시태그가 없습니다</span>
										</c:if>
															...
															<c:if test="${reply.ref_level eq 0 }">
																...
																<c:if test="${reply.rpCheck ne 'y'}">
																	<a href="userProfile.do?nick=${reply.nick }"
																		...
																		class="rounded-circle"> <span class="text-muted">
																			${reply.nick} </span>
																	</a>
																</c:if>
						...
						$
								.ajax({
									...
									success : function(replyList) {
										...
										listHtml += "<a href='feedDetail.do?anNum="
												+ anNum
												+ "' class='text-muted' style='text-decoration: none;'>댓글 "
												+ replyList.length
												+ "개 전체보기</a></div><br>";
											...
											success : function(list) {
												...
												if (list == null) {
													...
													for ( const i in list) {
														...
														listHtml += "<h3 class='card-header'>"
																+ anTitle
																+ "<a href='reportfeed.do?anNum="
																+ anNum
																+ "'class='btn btn-primary btn-sm mt-1' style='float: right;'>신고🚨</a></h3>";
														...
														listHtml += "<span id='like"+anNum+"'>"
																+ likeCount
																+ "</span> <span class='col-5'>";
														if (proNick == userNick) {
															listHtml += "<a class='btndel btn btn-primary btn-sm mt-1' style='float: right;' idx='"+anNum+"'>삭제</a>";
															listHtml += "<a href='anoFeedUpdateForm.do?anNum="
																	+ anNum
																	+ "' class='btn btn-primary btn-sm mt-1'	style='text-decoration: none; float: right;'>수정</a>";
														}
														listHtml += "	</span></div><div class='card-footer'>";
														if (hashTagList != null) {
															for ( const j in hashTagList) { // 
																let tag = hashTagList[j].tag;
																listHtml += "<span class='btn btn-primary btn-sm mt-1'>#"
																		+ tag
																		+ "</span>";
															}
														} else {
														...
														listHtml += "<div class='card-body'> <pre>"
																+ anContent
																+ "</pre></div>";
