			...
			<div class="col-lg-4 mb-2">
				<div class="page-section mb-2">
					<div>						
							...
							<div class="accordion-item mb-2">
								<h2 class="accordion-header" id="headingOne">
									<button class="accordion-button collapsed" type="button"
										...
										aria-expanded="false" aria-controls="collapseOne">피드
										공개조건</button>
								</h2>
								<form action="searchCondition.do">
									...
									<div id="collapseOne" class="accordion-collapse collapse"
										...
										<div class="accordion-body col-12">
											...
											<div class="row">
												<strong class="mb-1">태그 선택</strong><br> <input
													class="form-control mb-1 mt-1" type="text" id="filterText"
													placeholder="원하는 태그 검색">
													<c:if
														test="${showCondition.anHash eq null || showCondition.anHash eq 'none' }">
														<div class="search col-lg-4">
															<input class="btn-check" type="radio" value="none"
																id="none" name="anHash" checked="checked"><label
																class="btn btn-outline-secondary btn-sm mt-1" for="none">상관없음</label>
														</div>
													</c:if>
													<c:if test="${showCondition.anHash ne 'none'}">
														<div class="search col-lg-4">
															<input class="btn-check" type="radio" value="none"
																id="none" name="anHash"><label
																class="btn btn-outline-secondary btn-sm mt-1" for="none">상관없음</label>
														</div>
													</c:if>
													<c:set value="${showCondition.anHash}" var="temptag"></c:set>
													<c:forEach var="tagt" items="${tags }">
														<c:if test="${tagt eq temptag}">
															<div class="search col-lg-4">
																<input class="btn-check" type="radio" value="${tagt }"
																	id="${tagt }" name="anHash" checked="checked">
																<label class="btn btn-outline-secondary btn-sm mt-1"
																	for="${tagt }">${tagt }</label>
															</div>
														</c:if>
														<c:if test="${tagt ne temptag}">
															<div class="search col-lg-4">
																<input class="btn-check" type="radio" value="${tagt }"
																	id="${tagt }" name="anHash"><label
										...
										<div class="accordion-body">
											<fieldset class="form-group">
												<strong>거리 기준</strong> <br>
												<c:if
													test="${showCondition.distCheck eq null  || showCondition.distCheck eq 'n'}">
													<input class="btn-check" type="radio" value="n" id="n"
														name="distCheck" checked="checked">
													<label class="btn btn-outline-secondary btn-sm mt-1"
														for="n">상관없음</label>
													<input class="btn-check" type="radio" value="y" id="y"
														name="distCheck">
													<label class="btn btn-outline-secondary btn-sm mt-1"
														for="y">내주변</label>
												</c:if>
												<c:if
													test="${showCondition.distCheck ne null  && showCondition.distCheck eq 'y'}">
													<input class="btn-check" type="radio" value="n" id="n"
														name="distCheck">
													<label class="btn btn-outline-secondary btn-sm mt-1"
														for="n">상관없음</label>
						...
						<div class="col-3">
							<a href="feedWriteForm.do" class="btn btn-primary btn-sm mb-3">새
								피드</a>
						</div>
					</div>
					<div id="appendPoint">
						...
						<c:if test="${anoFeed.anDel ne 'y' }">
							<div class="card mb-3">
								<h3 class="card-header">${anoFeed.anTitle }
									<a href="reportfeed.do?anNum=${anoFeed.anNum }"
                ...
                $.ajax({
					...
					success : function(list) {
						...
						for ( const i in list) {
							...
							listHtml += "<h3 class='card-header'>"+anTitle+"<a href='reportfeed.do?anNum="+anNum+"'class='btn btn-primary btn-sm mt-1' style='float: right;'>신고🚨</a></h3>";
							...
							if(proNick == userNick) {
								...
								listHtml += "<a href='anoFeedUpdateForm.do?anNum="+anNum+"' class='btn btn-primary btn-sm mt-1'	style='text-decoration: none; float: right;'>수정</a>";
							}
